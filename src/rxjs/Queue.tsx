...
export default function Queue() {
  ...
  return (
    <div>
      <div className="text-3xl mb-2">RxJs 队列实现</div>
      <div className="mb-2">
        <Button onClick={click} size="large" type="primary">
          push
        </Button>
      </div>
      <div
        className={
          "h-[100px] border-solid border-amber-800 border-2" +
...
const writeBall = (num: number, content: HTMLDivElement) => {
  const color =
    "rgba(" +
    [Math.random() * 256, Math.random() * 256, Math.random() * 256].join(",") +
    ", 1)";
